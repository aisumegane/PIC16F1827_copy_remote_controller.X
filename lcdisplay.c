...
{   /*シーケンスによって表示内容変更*/  /*copyモードでは極力割込みさせたくないので、モードに入る前に表示処理を実行する*/
    switch(sequence_num)
    {
        case SEQUENCE_MAIN:
            ...
            lcdisplay_cmmand_8bit_write(copydata_choose,DATA_WRITE);
            lcdisplay_set_cursor(SECOND_ROW,FIRST_COLUMN);
            lcdisplay_cmmand_8bit_write(senddata_choose,DATA_WRITE);
...
{   
    ...
    /*================== LCD function setup ====================*/
    lcdisplay_cmmand_4bit_commandwrite(0x02,COMMAND_WRITE);  /*4bit通信選択時のみ、4bit送信を1回行う*/  /*この時のRS信号は？*/
...
{   /*PCF8574の4bit通信フォーマットに合わせてデータを改造する*/
    ...
    data_high4bit = data_high4bit >> 4;
    
    lcdisplay_cmmand_4bit_commandwrite(data_high4bit,data_command_select);  /*higher 4bit data*/
    lcdisplay_cmmand_4bit_commandwrite(data,data_command_select);           /*lower  4bit data*/
}
